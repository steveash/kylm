...
package com.github.steveash.kylm.model.ngram.smoother;
...
public class AbsoluteSmoother extends NgramSmoother {
    private static final Logger log = LoggerFactory.getLogger(AbsoluteSmoother.class);
    ...
    public void smooth(NgramLM lm) throws Exception {
        ...
        log.debug("AbsoluteSmoother: calculating FOFs");
        ...
        log.debug("AbsoluteSmoother: interpolating model");
        ...
        if (i != 0) {
            for (NgramNode child : node)
                if (child != null && child.getScore() != NgramNode.TRIM_SCORE)
                    child.setScore(KylmMathUtils.logAddition(child.getScore(), node.getBackoffScore() + child.getFallback()
                            .getScore()));
