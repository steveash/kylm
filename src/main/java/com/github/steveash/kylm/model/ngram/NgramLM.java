...
import com.github.steveash.kylm.model.ngram.smoother.NgramSmoother;
import com.github.steveash.kylm.reader.TextArraySentenceReader;
...
public class NgramLM extends LanguageModel implements Serializable {
    private static final Logger log = LoggerFactory.getLogger(NgramLM.class);
...
//			mids = iids;
    ...
    public void trainModel(Iterable<String[]> sl) throws Exception {
        log.info("NgramLM.trainModel(): Started for {}", name);
        // count the n-grams
        countNgrams(sl);
        // smooth the n-grams
        if (smoother != null)
        ...
        log.info("NgramLM.trainModel(): Finished for {}", name);
    }
    ...
    public void countNgrams(Iterable<String[]> sl) throws IOException {
        log.info("NgramLM.countNgrams(): Started for {}", name);
        ...
        for (String[] sent : sl) {
            // output progress
            if (++count % 100_000 == 0) {
                log.debug("Completed {} sentences for ngram counting", count);
            }
    ...
    public void expandUnknowns() {
        ...
        log.debug("Expanding {} unknown words", vec.size());
    ...
    public boolean equals(Object o) {
        ...
        NgramLM ngramLM = (NgramLM) o;

        if (n != ngramLM.n) return false;
        if (sentHits != ngramLM.sentHits) return false;
        if (!Arrays.equals(counts, ngramLM.counts)) return false;
        if (root != null ? !root.equals(ngramLM.root) : ngramLM.root != null) return false;
        if (smoother != null ? !smoother.equals(ngramLM.smoother) : ngramLM.smoother != null) return false;
        if (!Arrays.equals(hits, ngramLM.hits)) return false;
        return ukWords != null ? ukWords.equals(ngramLM.ukWords) : ngramLM.ukWords == null;
    }
    ...
    public int hashCode() {
        ...
        result = 31 * result + (smoother != null ? smoother.hashCode() : 0);
        result = 31 * result + Arrays.hashCode(hits);
        result = 31 * result + sentHits;
